    ...
    while runner.t_env <= args.t_max:
        ...
        if (runner.t_env - last_test_T) / args.test_interval >= 1.0:
            ...
            for _ in range(n_test_runs):
                runner.run(test_mode=True)
        #################### This version of model saving is for env: `SUMO_intersection_CAVs_flow` ##########################
        ...
        #################### This version of model saving is for env: `SUMO_intersection_random` ##########################
        if args.save_model and (runner.return_mean > args.max_return):
            # model_save_time = runner.t_env
            save_path = os.path.join(args.local_results_path, "models", args.unique_token, str(runner.t_env))
            # "results/models/{}".format(unique_token)
            os.makedirs(save_path, exist_ok=True)
            ...
            # use appropriate filenames to do critics, optimizer states
            learner.save_models(save_path)
            args.max_return = runner.return_mean
